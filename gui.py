...
# --- ЗМІНА: Спрощуємо цей імпорт, REFERENCE_CITY більше не потрібен тут ---
...
class CalculatorGUI(QWidget):
    ...
    def init_ui(self):
        ...
        self.tracked_coins = { "BTCUSDT": "BTC", "ETHUSDT": "ETH", "SOLUSDT": "SOL", "BNBUSDT": "BNB", "LINKUSDT": "LINK", "OPUSDT": "OP" }
        self.coin_labels = {}
        for symbol, name in self.tracked_coins.items():
            label = QLabel(f"{name}: Завантаження...")
    ...
    def update_prices(self):
        for symbol, label in self.coin_labels.items():
            ...
            if data:
                ...
                name = symbol[:-4]
                label.setText(f"{name}: Немає даних")

    def update_times(self):
        exchange_status = {}
        # Використовуємо Лондон як референтне місто для визначення вихідного
        ref_now = datetime.now(pytz.timezone(CITIES_TIMEZONES["Лондон"]))
        ...
        for city in CITIES_TIMEZONES.keys():
            ...
            if is_weekend:
                ...
                status = "Active" if is_exchange_active(city) else "Passive"
        ...
        for city, info in sorted_exchanges:
            ...
            if info["status"] == "Active":
                self.countdown_labels[city].setText("Зараз торги")
            else:
                self.countdown_labels[city].setText(f"До відкриття: {info['time_until_open']}")
