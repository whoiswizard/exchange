...
# --- ЗМІНА: Імпортуємо функції з нашого нового файлу database.py ---
...
class CalculatorGUI(QWidget):
    def __init__(self):
        ...
        # --- ЗМІНА: Ініціалізація та наповнення БД при першому запуску ---
        init_db()
        ...
        # Словник для зберігання віджетів монет (чекбокс, лейбл)
        self.coin_widgets = {}
        
        self.init_ui()
        self.init_timers()
        
        # --- ЗМІНА: Завантажуємо та відображаємо монети з БД ---
        self.refresh_coin_display()

    def init_ui(self):
        main_layout = QVBoxLayout()
        # ... (Код для калькулятора залишається без змін, тому я його приховаю для стислості)
        ...
        # --- ЗМІНА: Оновлюємо панель з кнопками ---
        ...
        top_bar_layout.addStretch() # Розтягувач, щоб кнопки були зліва
        ...
        # ... (Код для блоку часу залишається без змін)
    ...
    def init_timers(self):
        # Прибираємо автоматичне оновлення, залишаємо лише таймер для годинників
        self.time_timer = QTimer()
        self.time_timer.timeout.connect(self.update_times)
        self.time_timer.start(1000)
        self.update_times()

    # --- НОВИЙ МЕТОД: Повністю перебудовує список монет з БД ---
    def refresh_coin_display(self):
        # Очищуємо старі віджети
        for symbol in list(self.coin_widgets.keys()):
            widgets = self.coin_widgets.pop(symbol)
            widgets['layout'].deleteLater()

        # Отримуємо актуальний список монет з БД
        symbols = get_coins()
        
        # Створюємо нові віджети
        ...
        # Оновлюємо ціни для новостворених лейблів
        self.update_prices()

    def update_prices(self):
        # Тепер ітеруємо по віджетах, а не по окремому списку
        for symbol, widgets in self.coin_widgets.items():
            ...
            if data:
                ...
                # ... (решта логіки форматування та відображення залишається такою ж)
    ...
    # --- НОВИЙ МЕТОД: Видаляє обрані монети ---
    def delete_selected_coins(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            # Після видалення з БД, оновлюємо відображення
            self.refresh_coin_display()
    
    # --- ЗМІНА: Логіка додавання монети тепер працює з БД ---
    def add_coin(self):
        ...
        # Оновлюємо весь список, щоб додати новий елемент
        self.refresh_coin_display()
    
    # ... (решта методів: calculate, update_times, format_volume і т.д. залишаються без змін)
    def calculate(self):
        try:
